import OBR, { Item, isCurve, isImage } from "@owlbear-rodeo/sdk";
import { BSCACHE } from "./bsSceneCache";

export class ViewportFunctions
{
    static async FocusUnit(event: MouseEvent): Promise<void>
    {
        event.preventDefault();

        const row = (event.target as HTMLElement).closest('tr') as HTMLTableRowElement;
        const unitId = row.getAttribute("unit-id");
        const unit = BSCACHE.sceneItems.find(item => item.id === unitId)!;

        await ViewportFunctions.CenterViewportOnImage(unit);
    }
     ...
     */
    static async CenterViewportOnImage(item: Item)
    {
        ...
        const imageCenter = await this.GetImageCenter(item, dpi);
